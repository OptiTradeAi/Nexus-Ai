...
logger.info("UI_PUBLIC path: %s", UI_PUBLIC)
...
try:
    ...
    logger.warning("transcribe_service not available; upload_audio disabled.")

try:
    from core import kaon_engine as kaon_engine
except Exception:
    kaon_engine = None
    logger.info("core.kaon_engine not found; optional integration.")
...
async def ui_dashboard(request):
    if not is_logged_in(request):
        return web.HTTPFound('/ui/index.html')
    return await serve_file("dashboard.html")
...
async def ws_screen_handler(request):
    ...
    logger.info("🔌 WS client connected /ws_screen")
    ...
    async for msg in ws:
        try:
            if msg.type == web.WSMsgType.TEXT:
                ...
                logger.error("WS closed with exception %s", ws.exception())
        except Exception as e:
            logger.exception("WS message error")
            await ws.send_json({"status": "error", "error": str(e)})

    logger.info("❌ WS client disconnected /ws_screen")
    return ws
...
async def api_test_candle(request):
    try:
        ...
        logger.info('📩 /api/test_candle: %s', data)
...
async def upload_audio_handler(request):
    ...
    out_path = UPLOADS_DIR / os.path.basename(filename)
...
def create_app():
    ...
    # Serve ALL files in public recursively
    if UI_PUBLIC.exists():
    ...
    routes = [
        ('/', index_redirect, 'GET'),
        ('/ui/index.html', ui_index, 'GET'),
        ('/ui/dashboard.html', ui_dashboard, 'GET'),
        ('/login', login_post, 'POST'),
        ('/logout', logout_get, 'GET'),
        ('/health', health_check, 'GET'),
        ('/ws_screen', ws_screen_handler, 'GET'),
        ('/ws_stream', ws_screen_handler, 'GET'),
        ('/api/test_candle', api_test_candle, 'POST'),
    ]
    for path, handler, method in routes:
        res = app.router.add_resource(path)
        route = res.add_route(method, handler)
        cors.add(route)

    # Upload / transcription
    if transcribe_service:
        extra_routes = [
            ('/upload_audio', upload_audio_handler, 'POST'),
            ('/transcriptions/{name}', get_transcription_handler, 'GET'),
            ('/transcriptions', list_transcriptions, 'GET'),
        ]
        for path, handler, method in extra_routes:
            res = app.router.add_resource(path)
            route = res.add_route(method, handler)
            cors.add(route)
