...
# Paths
BASE_DIR = Path(__file__).resolve().parent   # nexus_ai/server
# NOTA: Com o Dockerfile na raiz, PROJECT_ROOT é '/' dentro do container.
PROJECT_ROOT = Path("/") 
UI_PUBLIC = Path("/web/ui/public") # Este é o caminho onde o Dockerfile COPIOU os arquivos

# AVISO: As definições de caminho local (BASE_DIR, PROJECT_ROOT) não são mais úteis no container,
# mas vamos re-definir UI_PUBLIC usando o caminho absoluto garantido pela nossa correção no Dockerfile.

# Ensure folders (Ajustadas para usar caminhos absolutos se estiver rodando no container)
# NOTA: O server.py está sendo copiado para /app. 
...
try:
    # Como o Dockerfile copia a pasta server/ para /app, 
    # e as outras pastas (core, etc.) NÃO estão em /app, esta importação 
    # pode precisar de um ajuste de PYTHONPATH, mas vamos manter o original 
    # por agora, assumindo que kaon_engine está em algum lugar acessível.
...
def create_app():
    ...
    # Mapeia a URL /ui/ para a pasta /web/ui/public.
    # Esta rota é crítica, pois UI_PUBLIC contém index.html, dashboard.html e assets.
    # O show_index=True garante que o aiohttp procure e sirva o index.html como padrão.
    if UI_PUBLIC.exists():
    ...
    # A rota original que servia apenas /ui/static (mantida, mas agora é redundante se a rota acima funcionar)
    # Serve static assets (css/js/images) from UI static folder
    ...
    # NOTA: Esta rota continua funcionando para o redirecionamento.
